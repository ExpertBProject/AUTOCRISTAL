...
#End Region
    ...
    Private Sub cargaCampos()
        If objGlobal.refDi.comunes.esAdministrador Then
            ...
            oXML = objGlobal.funciones.leerEmbebido(Me.GetType(), "UDO_EXO_CONAG.xml")
            objGlobal.refDi.comunes.LoadBDFromXML(oXML)
            objGlobal.SBOApp.StatusBar.SetText("Validado: UDO_EXO_CONAG", SAPbouiCOM.BoMessageTime.bmt_Medium, SAPbouiCOM.BoStatusBarMessageType.smt_Success)
        End If
    End Sub
        ...
        Try
            ...
            If oForm.Visible = True Then
                ...
                If _sIC <> "" Then
                    ...
                    oForm.DataSources.DBDataSources.Item("@EXO_CONAG").SetValue("Code", 0, _sIC)
                    oForm.DataSources.DBDataSources.Item("@EXO_CONAG").SetValue("Name", 0, _sDes)
                End If
        ...
        Try
            ...
            If Not oDataTable Is Nothing Then
                Select Case oForm.ChooseFromLists.Item(oCFLEvento.ChooseFromListUID).ObjectType
                    Case "EXO_CONDUCTORES"
                        Try
                            Dim sDes As String = oDataTable.GetValue("Name", 0).ToString
                            Dim sDes2 As String = oDataTable.GetValue("U_EXO_APELL", 0).ToString
                            oForm.DataSources.DBDataSources.Item("@EXO_CONAGL").SetValue("U_EXO_NOMBRE", pVal.Row - 1, oDataTable.GetValue("Name", 0).ToString)
                            ...
                            oForm.DataSources.DBDataSources.Item("@EXO_CONAGL").SetValue("U_EXO_APE", pVal.Row - 1, oDataTable.GetValue("U_EXO_APELL", 0).ToString)
                            CType(CType(oForm.Items.Item("0_U_G").Specific, SAPbouiCOM.Matrix).Columns.Item("C_0_3").Cells.Item(pVal.Row).Specific, SAPbouiCOM.EditText).Value = sDes2
                        Catch ex As Exception
                            oForm.DataSources.DBDataSources.Item("@EXO_CONAGL").SetValue("U_EXO_NOMBRE", pVal.Row - 1, oDataTable.GetValue("Name", 0).ToString)
                            oForm.DataSources.DBDataSources.Item("@EXO_CONAGL").SetValue("U_EXO_APE", pVal.Row - 1, oDataTable.GetValue("U_EXO_APELL", 0).ToString)
                        End Try
