...
#Region "variables"
        ...
        Dim sAgrupar As String = "N" : Dim sAlmacen As String = "" : Dim sAlmacenes As String = ""
...
#End Region
        ...
        Try
            ...
            sAlmacen = oForm.DataSources.UserDataSources.Item("UDALM").ValueEx.ToString
...
#Region "Agrupado"
                                If sAlmacen = "" Then
                                    sMensaje = "No tiene indicado el almacén principal. Es obligatorio indicar uno."
                                    objGlobal.SBOApp.StatusBar.SetText("(EXO) - " & sMensaje, SAPbouiCOM.BoMessageTime.bmt_Short, SAPbouiCOM.BoStatusBarMessageType.smt_Warning)
                                    objGlobal.SBOApp.MessageBox(sMensaje)
                                    Return False
                                End If
                                ...
                                For a As Integer = 0 To dtArticulos.Rows.Count - 1
                                    ...
                                    sSQLGrid &= " (SELECT SUM(IFNULL(""OpenQty"",0)) ""OpenQty"" FROM POR1 where ""LineStatus""<>'C' and ""WhsCode"" in (" & sAlmacenes & ") and ""ItemCode""='" & dtArticulos.Rows(a).Item("ItemCode").ToString & "') "
                                    sSQLGrid &= " UNION ALL "
                                    sSQLGrid &= " (SELECT SUM(IFNULL(""OpenQty"",0)) ""OpenQty"" FROM WTQ1 where ""LineStatus""<>'C' and ""WhsCode"" in (" & sAlmacenes & ") and ""WhsCode""<>""FromWhsCod"" and ""ItemCode""='" & dtArticulos.Rows(a).Item("ItemCode").ToString & "') "
                                    sSQLGrid &= ") T) ""PTE " & sAlmacen & """ "
                                    sSQLGrid &= ", (CASE WHEN (SELECT SUM(IFNULL(PCH1.""Quantity"",0)) FROM PCH1 INNER JOIN OPCH ON PCH1.""DocEntry"" = OPCH.""DocEntry""  "
                                    ...
                                    sSQLGrid &= " (SELECT SUM(IFNULL(RPC1.""Quantity"",0)) FROM RPC1 INNER JOIN ORPC ON RPC1.""DocEntry"" = ORPC.""DocEntry""  "
                                    ...
                                    sSQLGrid &= " And "
                                    sSQLGrid &= " (SELECT SUM(IFNULL(INV1.""Quantity"",0)) FROM INV1 INNER JOIN OINV ON INV1.""DocEntry"" = OINV.""DocEntry"" "
                                    sSQLGrid &= " WHERE OINV.""CANCELED""='N' and OINV.""DocDate"" <='" & dFecha.Year.ToString("0000") & dFecha.Month.ToString("00") & dFecha.Day.ToString("00") & "' "
                                    sSQLGrid &= " And OINV.""DocDate"">='" & dFechaAnt.Year.ToString("0000") & dFechaAnt.Month.ToString("00") & dFechaAnt.Day.ToString("00") & "' "
                                    sSQLGrid &= " And INV1.""WhsCode"" in (" & sAlmacenes & ") and INV1.""ItemCode""='" & dtArticulos.Rows(a).Item("ItemCode").ToString & "')- "
                                    ...
                                    'sSQLGrid &= " WHERE OPDN.""DocDate""<='" & dFecha.Year.ToString("0000") & dFecha.Month.ToString("00") & dFecha.Day.ToString("00") & "' "
                                    'sSQLGrid &= " And OPDN.""DocDate"">='" & dFechaSemestre.Year.ToString("0000") & dFechaSemestre.Month.ToString("00") & dFechaSemestre.Day.ToString("00") & "' "
                                    'sSQLGrid &= " And PDN1.""WhsCode"" in (" & sAlmacenes & ") and PDN1.""ItemCode""='" & dtArticulos.Rows(a).Item("ItemCode").ToString & "')>0 "
                                    'sSQLGrid &= " And (SELECT SUM(DLN1.""OpenQty"") FROM DLN1 INNER JOIN ODLN ON DLN1.""DocEntry"" = ODLN.""DocEntry"" "
                                    'sSQLGrid &= " WHERE ODLN.""DocDate"" <='" & dFecha.Year.ToString("0000") & dFecha.Month.ToString("00") & dFecha.Day.ToString("00") & "' "
                                    'sSQLGrid &= " And ODLN.""DocDate"">='" & dFechaAnt.Year.ToString("0000") & dFechaAnt.Month.ToString("00") & dFechaAnt.Day.ToString("00") & "' "
                                    'sSQLGrid &= " And DLN1.""WhsCode"" in (" & sAlmacenes & ") and DLN1.""ItemCode""='" & dtArticulos.Rows(a).Item("ItemCode").ToString & "')=0 "
                                    'sSQLGrid &= " then 'SI' else 'NO' END) ""N " & sAlmacen & """ "
                                ...
                                For a As Integer = 0 To dtArticulos.Rows.Count - 1
                                    ...
                                    For i As Integer = 0 To oForm.DataSources.DataTables.Item("DTALM").Rows.Count - 1
                                        If oForm.DataSources.DataTables.Item("DTALM").GetValue("Sel", i).ToString = "Y" Then
                                            ...
                                            sSQLGrid &= " ,(SELECT IFNULL(""OnHand"",0) FROM OITW WHERE ""WhsCode""='" & sAlmacen & "' and ""ItemCode""='" & dtArticulos.Rows(a).Item("ItemCode").ToString & "') ""Stock " & sAlmacen & """ "
                                            sSQLGrid &= " , SELECT SUM(""OpenQty"") FROM ("
                                            sSQLGrid &= " (SELECT IFNULL(SUM(""OpenQty""),0) ""OpenQty"" FROM POR1 where ""LineStatus""<>'C' and ""WhsCode""='" & sAlmacen & "' and ""ItemCode""='" & dtArticulos.Rows(a).Item("ItemCode").ToString & "') "
                                            sSQLGrid &= " UNION ALL "
                                            sSQLGrid &= " (SELECT IFNULL(SUM(""OpenQty""),0) ""OpenQty"" FROM WTQ1 where ""LineStatus""<>'C' and ""WhsCode""='" & sAlmacen & "' and ""WhsCode""<>""FromWhsCod"" and ""ItemCode""='" & dtArticulos.Rows(a).Item("ItemCode").ToString & "') "
                                            sSQLGrid &= ") T ""PTE " & sAlmacen & """ "
                                            sSQLGrid &= ",(SELECT IFNULL(SUM(""OpenQty""),0) FROM POR1 where ""LineStatus""<>'C' and ""WhsCode""='" & sAlmacen & "' and ""ItemCode""='" & dtArticulos.Rows(a).Item("ItemCode").ToString & "') ""PTE " & sAlmacen & """ "
                ...
                Case "chkAgrupar"
                    Select Case sAgrupar
                        Case "Y"
                            ...
                            'oForm.Items.Item("txtProv").Enabled = True : CType(oForm.Items.Item("txtProv").Specific, EditText).ChooseFromListUID = "CFLPROV"
                        Case "N"
                            ' oForm.Items.Item("txtProv").Enabled = False
