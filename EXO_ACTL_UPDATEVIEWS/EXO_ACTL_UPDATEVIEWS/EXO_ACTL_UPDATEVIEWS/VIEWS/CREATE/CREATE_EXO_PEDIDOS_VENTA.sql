CREATE VIEW "BBDD"."EXO_PEDIDOS_VENTA" ( "T. SALIDA",
	 "DELEGACIÓN",
	 "FECHA CREACION",
	 "FECHA ENTREGA",
	 "Nº INTERNO",
	...
	END) AS "COM",
	 ...
	 T1."Territory",
	 --(CASE WHEN (SELECT
 --COUNT(1) FROM ORDR X0 WHERE X0."CardCode" = T0."CardCode" AND X0."DocEntry" <> T0."DocEntry" 		AND X0."DocStatus" = 'O') > 1 
 -- OR (SELECT	 COUNT(1) 
 --	FROM ODLN X0 WHERE X0."CardCode" = T0."CardCode" AND X0."DocDate" = CURRENT_DATE) > 1 THEN 'Y' ELSE 'N' END) "A",
 A."A" as "A",
	 CAST(IFNULL((SELECT
...
AND IFNULL("Pick"."RelQtty",
	 0)< TL."Quantity" 
AND T0."Confirmed" = 'Y' 
AND OITM."InvntItem" = 'Y' WITH READ ONLY
